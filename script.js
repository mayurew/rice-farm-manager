...
const transactionGroups = [
    ...
    { id: "overhead", label: "ค่าใช้จ่ายส่วนกลาง" },
    { id: "asset", label: "ทรัพย์สิน" }
];

const categories = {
    field_cost: [
        { id: "land", label: "เตรียมดิน" },
        { id: "seed", label: "เมล็ดพันธุ์" },
        { id: "fertilizer", label: "ปุ๋ย" },
        { id: "chemical", label: "ยา" },
        { id: "machine", label: "เครื่องจักร" },
        { id: "labor", label: "แรงงาน" },
    ...
    income: [
        { id: "rice_sale", label: "ขายข้าว" },
        { id: "straw_sale", label: "ขายฟางข้าว" },
        { id: "subsidy", label: "เงินอุดหนุนรัฐ" },
        { id: "other_income", label: "รายรับอื่น ๆ" }
    ],
    overhead: [
        { id: "repair", label: "ซ่อมเครื่องจักร" },
        { id: "phone", label: "โทรศัพท์" },
        { id: "travel", label: "เดินทาง" },
        { id: "fuel_transport", label: "น้ำมันขนส่ง" },
        { id: "admin", label: "งานเอกสาร" },
        { id: "office", label: "สำนักงาน" },
        { id: "other_overhead", label: "อื่น ๆ" }
    ]
};
...
function getCategoryLabel(group, categoryId) {
    const groupCategories = categories[group] || [];
    const category = groupCategories.find(item => item.id === categoryId);
    return category ? category.label : "ไม่ระบุหมวด";
}

function getGroupLabel(groupId) {
    const group = transactionGroups.find(item => item.id === groupId);
    return group ? group.label : "ไม่ระบุกลุ่ม";
}

function getFieldName(fieldId) {
    const field = fields.find(item => item.id === Number(fieldId));
    return field ? field.name : "ไม่ระบุแปลง";
}
...
function loadData() {
    ...
    fieldCosts = JSON.parse(localStorage.getItem("fieldCosts") || "[]");
    incomeRecords = JSON.parse(localStorage.getItem("incomeRecords") || "[]");
    overheadCosts = JSON.parse(localStorage.getItem("overheadCosts") || "[]");
...
function migrateOldExpenses() {
    ...
    if (oldExpenses.length === 0 || fieldCosts.length > 0) return;

    fieldCosts = oldExpenses
...
function isSameTransaction(group, item, deletedItem) {
    if (deletedItem.group !== group) return false;

    const sameId = String(item.id) === String(deletedItem.id);
    const sameContent = getTransactionName(item) === deletedItem.name &&
...
function removeDeletedLegacyExpenses() {
    ...
    const keptExpenses = oldExpenses.filter(item =>
        !deletedTransactions.some(deletedItem =>
            isSameTransaction(
    ...
    localStorage.setItem("expenses", JSON.stringify(keptExpenses));
}
...
function saveField() {
    ...
    if (editingFieldId) {
        ...
        editingFieldId = null;
        getElement("fieldSaveBtn").textContent = "เพิ่มแปลงนา";
    } else {
        fields.push({
            id: createId(),
            name,
...
function editField(id) {
    ...
    editingFieldId = id;
    getElement("fieldSaveBtn").textContent = "บันทึกการแก้ไข";
}
...
function deleteField(id) {
    ...
    const confirmed =
        confirm(`ลบแปลงนา "${field.name}" และต้นทุนของแปลงนี้?`);
...
function renderFields() {
    ...
    if (fields.length === 0) {
        fieldList.innerHTML =
            `<li class="empty-state">ยังไม่มีแปลงนา</li>`;
        return;
    }
        ...
        li.innerHTML = `
            <div class="list-row">
                <div class="list-info">
                    ...
                    <span>${formatNumber(field.area)} ไร่</span>
                    <small>${escapeHTML(field.location)}</small>
                </div>

                <div class="row-actions">
                    <button onclick="editField(${field.id})">แก้ไข</button>
                    <button onclick="deleteField(${field.id})">ลบ</button>
                </div>
            </div>
        `;
...
function updateIncomeQuantityPlaceholders() {
    ...
    if (currentCategory === "rice_sale") {
        quantityInput.placeholder = "เช่น 1000";
        unitInput.placeholder = "กิโลกรัม";
    } else if (currentCategory === "straw_sale") {
        quantityInput.placeholder = "เช่น 120";
        unitInput.placeholder = "ก้อน";
    } else {
        quantityInput.placeholder = "เช่น 1";
        unitInput.placeholder = "รายการ";
    }
}
...
function formatQuantityUnit(item) {
    ...
    return ` • ${formatNumber(item.quantity)} ${item.unit || "รายการ"}`;
}
...
function renderFieldSelect() {
    ...
    if (fields.length === 0) {
        fieldSelect.innerHTML =
            `<option value="">เพิ่มแปลงนาก่อน</option>`;

        fieldSelect.disabled = true;

        if (fieldHint) {
            fieldHint.innerHTML =
                `ไปที่หน้า <a href="field.html">แปลงนา</a> เพื่อเพิ่มแปลงแรก`;
        }
    ...
    fields.forEach(field => {
        ...
        option.textContent =
            `${field.name} (${formatNumber(field.area)} ไร่)`;
...
function saveFieldCost() {
    ...
    if (!fieldId) {
        alert("กรุณาเลือกแปลงนา");
        return;
    }

    if (!name || amount <= 0) {
        alert("กรอกข้อมูลต้นทุนแปลงนาให้ครบ");
        return;
    }
    ...
    const data = {
        ...
        unit: getElement("itemUnit").value.trim() || "รายการ",
...
function saveOverheadCost() {
    ...
    if (!name || amount <= 0) {
        alert("กรอกข้อมูลค่าใช้จ่ายส่วนกลางให้ครบ");
        return;
    }
...
function saveAsset() {
    ...
    if (!name || purchasePrice <= 0 || usefulLifeYears <= 0) {
        alert("กรอกข้อมูลทรัพย์สินให้ครบ");
        return;
    }
    ...
    const data = {
        ...
        purchasePrice,
        quantity: Number(getElement("itemQuantity").value || 1),
        unit: getElement("itemUnit").value.trim() || "ชิ้น",
...
function afterSaveTransaction() {
    getElement("saveBtn").textContent = "เพิ่มรายการ";
    clearTransactionForm();
...
function editTransaction(group, id) {
    ...
    getElement("saveBtn").textContent = "บันทึกการแก้ไข";
...
function renderTransactionList() {
    ...
    if (items.length === 0) {
        list.innerHTML =
            `<li class="empty-state">ยังไม่มีรายการ</li>`;
        return;
    }
...
function createTransactionHTML(item) {
    ...
    const detail =
        item.viewGroup === "field_cost"
            ? `${getFieldName(item.fieldId)} • ${getCategoryLabel("field_cost", item.category)}`
            : item.viewGroup === "income"
                ? `${getFieldName(item.fieldId)} • ${getCategoryLabel("income", item.category)}${formatQuantityUnit(item)}`
            : item.viewGroup === "overhead"
                ? getCategoryLabel("overhead", item.category)
                : `${formatNumber(item.quantity)} ${escapeHTML(item.unit)} • ${formatNumber(item.usefulLifeYears)} ปี`;

    return `
        <div class="list-row ${item.viewGroup}">
            <div class="list-info">
                <strong>${escapeHTML(item.name)}</strong>
                <span>${formatNumber(amount)} บาท</span>
                <small>${groupLabel}</small>
                <small>${escapeHTML(detail)}</small>
                <small>${escapeHTML(item.createdAt || item.purchaseDate || "ไม่มีข้อมูลวันที่")}</small>
            </div>

            <div class="row-actions">
                <button onclick="editTransaction('${item.viewGroup}', '${item.id}')">แก้ไข</button>
                <button onclick="deleteTransaction('${item.viewGroup}', '${item.id}')">ลบ</button>
            </div>
        </div>
    `;
}
...
function renderAssetList() {
    ...
    if (assets.length === 0) {
        assetList.innerHTML =
            `<li class="empty-state">ยังไม่มีทรัพย์สิน</li>`;
        return;
    }
        ...
        li.innerHTML = `
            <div class="list-row asset">
                <div class="list-info">
                    ...
                    <small>${formatNumber(asset.purchasePrice)} บาท</small>
                    <small>${formatNumber(asset.usefulLifeYears)} ปี</small>
                    <small>${escapeHTML(asset.purchaseDate)}</small>
                </div>

                <div class="row-actions">
                    <button onclick="deleteTransaction('asset', '${asset.id}')">ลบ</button>
                </div>
            </div>
        `;
...
function createPieChartHTML(parts) {
    ...
    if (total <= 0) {
        return `<div class="empty-state">ยังไม่มีข้อมูลสำหรับกราฟ</div>`;
    }
...
function renderFieldSummary() {
    ...
    if (fields.length === 0) {
        summaryList.innerHTML =
            `<div class="empty-state">ยังไม่มีแปลงนา</div>`;
        return;
        
        //คำนวณผลผลิตและกำไรต่อไร่สำหรับแต่ละแปลงนา
        const yieldPerRai =
    ...
    <p>
        ...
        กก./ไร่
            ...
            <div class="summary-grid">
                ...
                <div>
                    <span>ต้นทุน/ไร่</span>
                    <strong>${formatNumber(costPerRai)} บาท</strong>
                </div>
                <div>
                    <span>กำไร/ไร่</span>
                    <strong>${formatNumber(profitPerRai)} บาท</strong>
                </div>
                <div>
